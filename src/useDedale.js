...
const cellSize = 100;
const ballSize = 40;
const numberHoles = Math.floor(window.innerHeight / 100);
...
export default ({ xAcceleration, yAcceleration, width, height }) => {
  const [{ x, y, maze, goal, holes, level, lost, safe }, setState] = useState({
    x: 0,
    y: 0,
    maze: generateMaze(width / cellSize, height / cellSize),
    goal: { x: width / cellSize - 1, y: height / cellSize - 1 },
    holes: getHoles(width, height, numberHoles, [
      { x: 0, y: 0 },
      { x: width / cellSize - 1, y: height / cellSize - 1 },
    ]),
    level: 0,
    lost: false,
    safe: true,
  ...
  const retry = () => {
    ...
    setState({
      ...
      maze: generateMaze(width / cellSize, height / cellSize),
      goal: newGoal,
      holes: getHoles(width, height, numberHoles, [{ x: 0, y: 0 }, newGoal]),
      level: 0,
      lost: false,
      safe: true,
    });
  ...
  useEffect(() => {
    ...
    if (detectCircleCollision({ x, y }, goal)) {
      ...
      setState((state) => ({
        ...
        maze: generateMaze(width / cellSize, height / cellSize),
        goal: newGoal,
        holes: getHoles(width, height, numberHoles, [currentCell, newGoal]),
        level: level + 1,
        safe: true,
      }));
      return;
